...
def action_judge(boss_blood, next_boss_blood, self_blood, next_self_blood, stop, emergence_break, survival_steps, action=None):
    """动作奖励判断函数 - 增加存活时间奖励机制和翻滚惩罚，返回boss死亡状态"""
    # 基础存活奖励：每次决策都给予基础奖励，存活时间越长奖励越高
    base_survival_reward = 0  # 基础存活奖励
    survival_bonus = min(survival_steps * 0.03, 5)  # 存活步数奖励，最大2
    survival_reward = base_survival_reward +survival_bonus
    
    # 翻滚动作惩罚
    dodge_penalty = 0
    if action is not None and action in [2, 3]:  # action 2: dodge, action 3: rightdodge
        dodge_penalty = 0  # 翻滚负面奖励
        # print(f"翻滚动作惩罚: {dodge_penalty}")  # 调试信息
    elif action is not None and action == 0:  # action 1: None
        dodge_penalty = 0.2  # 不翻滚正面奖励

    # 判断玩家是否死亡（血量低于3）
    if next_self_blood < 3:
        if emergence_break < 2:
            # 死亡惩罚，但仍给予存活奖励（鼓励尽可能存活更久）
            reward = 0  # 死亡惩罚减少，保留存活奖励
            done = 1      # 训练结束标志
            stop = 0      # 重置停止标志
            emergence_break += 1
            boss_defeated = False  # 玩家死亡，boss未被击败
            print(f'玩家死亡，存活{survival_steps}步，存活奖励{survival_reward:.3f}')
            return reward, done, stop, emergence_break, boss_defeated
        else:
            reward =  0
            done = 1
            stop = 0
    ...
    # 判断Boss是否死亡
        ...
        if emergence_break < 2:
            # Boss死亡大奖励，加上存活奖励
            reward = 7+ survival_reward  # 增加Boss死亡奖励
            done = 1    # 训练继续
            stop = 0
            emergence_break += 1
            boss_defeated = True  # 明确标识boss被击败
            print(f'Boss死亡！存活{survival_steps}步，总奖励{reward:.3f}')
            return reward, done, stop, emergence_break, boss_defeated
        else:
            reward = 7+ survival_reward
            done = 0
            stop = 0
            emergence_break = 100
            boss_defeated = True  # 明确标识boss被击败
        ...
        if boss_blood - next_boss_blood > 10:
            boss_blood_reward =8 + survival_reward
        # elif action is not None and action == 1:
        #     boss_blood_reward = -0.5

        
        reward = self_blood_reward + boss_blood_reward + dodge_penalty  # 加上翻滚惩罚
        done = 0  
        emergence_break = 0  
        ...
        if survival_steps % 100 == 0:
            print(f'存活{survival_steps}步，当前存活奖励{survival_reward:.3f}')
...
DQN_log_path = os.path.join(script_dir, "logs_gpu")
...
if __name__ == '__main__':
    ...
    for episode in range(EPISODES):
        ...
        print(f"回合 {episode} 开始 - Boss血量: {boss_blood}, 玩家血量: {self_blood}")
        ...
        while True:
            ...
            if current_time - episode_start_time > 150: 
                print(f"回合 {episode} 超时触发紧急重启")
            ...
            if no_aim_count >= 30:
                print(f"光标失去超过30次({no_aim_count}次),停止训练并启动restart3")
                restart3()
                done = 0
                reward = 0
                break
            
            if not has_aim and no_aim_count < 20:
                print(f"光标丢失第{no_aim_count}次")
                
                # # 在跳过期间也检查是否死亡
                # if next_self_blood < 3:
                #     print("光标丢失期间检测到角色死亡，进入重启环节")
                #     done = 1
                #     reward = 0
                #     break  # 跳出训练循环，进入重启
                # elif next_boss_blood < 5 :
                #     print("光标丢失期间检测到boss死亡，进入重启环节")

                #     done = 1
                #     reward = 16
                #     # 计算回合时间并保存boss击败模型
                #     if not boss_victory_saved:
                #         episode_time = time.time() - episode_start_time
                #         avg_reward = total_reward/survival_steps if survival_steps > 0 else 0
                #         agent.save_boss_victory_model(episode, episode_time, avg_reward)
                #         boss_victory_saved = True
                #     break  # 跳出训练循环，进入重启
                # continue  # 跳过本次训练步骤，继续下一次循环
            
            print(f"血量变化 - Boss: {boss_blood}->{next_boss_blood}, 玩家: {self_blood}->{next_self_blood}")
            
            reward, done, stop, emergence_break, boss_defeated = action_judge(boss_blood, next_boss_blood,
                                                                self_blood, next_self_blood,
                                                                stop, emergence_break, survival_steps, action)
            
            print(f"动作: {action}, 奖励: {reward}")
            
            if boss_defeated and not boss_victory_saved:
                episode_time = time.time() - episode_start_time
                avg_reward_current = total_reward/survival_steps if survival_steps > 0 else 0
                agent.save_boss_victory_model(episode, episode_time, avg_reward_current)
                boss_victory_saved = True
                print("检测到boss死亡，已保存特殊模型")
            ...
            if target_step % UPDATE_STEP == 0:
                ...
                print(f"目标网络已更新 (步数: {target_step})")
        ...
        if timeout_triggered:
            print("执行超时处理流程：")
            print("1. 使用restart3进行紧急重启")
            print("2. 回退到上一个保存的模型")
            
            if episode > 0:
                backup_model_path = os.path.join(os.path.dirname(DQN_model_path), "pytorch_model.pth")
                if os.path.exists(backup_model_path):
                    if agent.load_model(backup_model_path):
                        print(f"已回退到上一个模型状态: {backup_model_path}")
                    else:
                        print("模型回退失败，继续使用当前模型")
                else:
                    print(f"备份模型不存在: {backup_model_path}，继续使用当前模型")
        ...
        if episode % 10 == 0  :
            agent.save_model()
            print(f"模型已保存 (回合 {episode})")
        
        avg_reward = total_reward/survival_steps if survival_steps > 0 else 0
        print(f'回合: {episode}, 存活步数: {survival_steps}, 评估平均奖励: {avg_reward:.4f}, Epsilon: {agent.epsilon:.4f}')
